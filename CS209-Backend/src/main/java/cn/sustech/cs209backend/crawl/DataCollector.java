...
public class DataCollector {
    ...
    private final DatabaseService databaseService = new DatabaseService();  // 数据库服务
...
//        refresh();
    ...
    public void collectData() throws IOException, SQLException {
        ...
        topics.add("io");
        topics.add("socket");
        topics.add("exception");
        topics.add("reflection");
        topics.add("javafx");
        topics.add("java-ee");
        topics.add("spring");
...
//                String params = String.format("page=%d&pagesize=%d&order=desc&sort=activity&tagged=java&filter=withbody&site=stackoverflow&key=gqjiH6ExBbic7NaMoFxC)w((", step, pageSize);
                String params = String.format("page=%d&pagesize=%d&order=desc&sort=activity&tagged=java;%s&filter=withbody&site=stackoverflow&key=gqjiH6ExBbic7NaMoFxC)w((", step, pageSize, topic);
            ...
            String QuestionPath = "/Users/cooperz/SUSTech/2023_Fall/CS209_Java2/Final-Project-repo/CS209-Backend/src/main/java/cn/sustech/cs209backend/crawl/data/questions_" + topic + ".json";
