<template>
  ...
  var myChart;
  const getBugsByView = () => {
    axios.get(`/exception/topKByAvgScore/8`, {}, {})
    .then(response => {
        items.value = response.data
        exception.value = items.value.map(item=>(item.averageScore))
        axios.get(`/fatalError/topKByAvgScore/8`, {}, {})
        .then(response => {
            items_fatalerror.value = response.data
            fatalError.value = items_fatalerror.value.map(item=>(item.averageScore))
            axios.get(`/syntaxError/topKByAvgScore/8`, {}, {})
            .then(response => {
                items_syntaxerror.value = response.data
                syntaxError.value = items_syntaxerror.value.map(item=>(item.averageScore))
