<template>
  ...
  var myChart;
  const getBugsByView = () => {
    axios.get(`/exception/topKByQuestionCount/8`, {}, {})
    .then(response => {
        items.value = response.data
        exception.value = items.value.map(item=>(item.questionCount))
        axios.get(`/fatalError/topKByQuestionCount/8`, {}, {})
        .then(response => {
            items_fatalerror.value = response.data
            fatalError.value = items_fatalerror.value.map(item=>(item.questionCount))
            axios.get(`/syntaxError/topKByQuestionCount/8`, {}, {})
            .then(response => {
                items_syntaxerror.value = response.data
                syntaxError.value = items_syntaxerror.value.map(item=>(item.questionCount))
