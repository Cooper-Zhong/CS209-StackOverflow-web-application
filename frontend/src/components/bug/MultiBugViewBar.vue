<template>
  ...
  var myChart;
  const getBugsByView = () => {
    axios.get(`/exception/topKByViewCount/8`, {}, {})
    .then(response => {
        items.value = response.data
        exception.value = items.value.map(item=>(item.average_view_count))
        axios.get(`/fatalError/topKByViewCount/8`, {}, {})
        .then(response => {
            items_fatalerror.value = response.data
            fatalError.value = items_fatalerror.value.map(item=>(item.average_view_count))
            axios.get(`/syntaxError/topKByViewCount/8`, {}, {})
            .then(response => {
                items_syntaxerror.value = response.data
                syntaxError.value = items_syntaxerror.value.map(item=>(item.average_view_count))
